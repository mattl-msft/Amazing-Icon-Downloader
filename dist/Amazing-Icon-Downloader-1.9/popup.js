		...
		for(let e=0; e<symbols.children.length; e++) {
			...
			*/
			...
			if(symbol.getElementsByTagName('path').length) {
				...
				} else if (pathd === 'M11 3.9l-.7-.8L6 7.4 1.7 3.1l-.7.8 5 5z') {
					nameMap[symbolID] = 'Chevron down';
				} else if (pathd === 'M7 0h1v16H7z') {
					nameMap[symbolID] = 'Pipe';
				} else if (pathd === 'M1 8.1l.7.8L6 4.6l4.3 4.3.7-.8-5-5z') {
					nameMap[symbolID] = 'Chevron up';
				} else if (pathd === 'M4 3.7l-4 4 .7.7L4 5l3.3 3.4.7-.7-4-4z') {
...
function populateIconList(list) {
	...
	if(elements.length) {
		...
		// Make the SVG code stand-alone by doing replacements
		...
		for(let i = elements.length-1; i > -1; i--) {
			...
			// Add the icon as a row in the main content
